  ...
  if (state.floorPlan?.objectUrl) {
    return `
      <section class="tool-card image-card" aria-labelledby="upload-title">
        ...
        <details class="overlay-demo"><summary>专业叠盘</summary><p>九宫、二十四山、玄空三盘将在确认宅向后叠加于户型图。</p><div class="overlay-controls"><label><input type="checkbox" data-overlay-layer="grid" checked>九宫</label><label><input type="checkbox" data-overlay-layer="mountains" checked>二十四山</label><label>大小<input type="range" data-overlay-scale min="0.6" max="1.5" step="0.05" value="${state.overlay.analysis.scale}"></label><label>透明度<input type="range" data-overlay-opacity min="0.25" max="1" step="0.05" value="${state.overlay.analysis.opacity}"></label><button type="button" data-overlay-action="flip">南北调转</button><button type="button" data-overlay-action="reset">复位叠盘</button></div></details>
